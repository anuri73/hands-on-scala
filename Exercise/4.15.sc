def isValidSudoku(array: Array[Array[Int]]): Boolean = {
    !Range(0, 9).exists { i =>
        val row = Range(0, 9).map(array(i)(_)).filter(_ != 0)
        val col = Range(0, 9).map(array(_)(i)).filter(_ != 0)
        val square = Range(0, 9).map(j => array((i % 3) * 3 + j % 3)((i / 3) * 3 + j / 3)).filter(_ != 0)
